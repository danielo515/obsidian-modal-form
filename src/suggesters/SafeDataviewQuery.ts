import { E, Either, parseAsyncFunctionBody, parseFunctionBody, pipe } from "@std";
...
export type SafeDataviewQuery = (
    dv: unknown,
    pages: unknown,
) => Either<ModalFormError, string[]> | Promise<Either<ModalFormError, string[]>>;
/**
...
export function sandboxedDvQuery(query: string): SafeDataviewQuery {
    ...
    if (query.includes("await")) {
        const parsed = parseAsyncFunctionBody<[unknown, unknown], unknown>(query, "dv", "pages");
        return async (dv: unknown, pages: unknown) => {
            try {
                const fnResult = await parsed;
                if (E.isLeft(fnResult)) {
                    return E.left(
                        new ModalFormError(
                            "Error evaluating the dataview query",
                            fnResult.left.message,
                        ),
                    );
                }
                const fn = fnResult.right;
                const result = await fn(dv, pages);
 ...
 */
export async function executeSandboxedDvQuery(
